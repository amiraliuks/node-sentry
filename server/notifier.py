...
# SSRF guards: only ever POST to a real Telegram/Discord endpoint, never an
# arbitrary attacker-supplied URL/host.
...
def _safe_ts(value):
    """Coerce an untrusted timestamp to a usable epoch (never let time.* raise)."""
...
_state_lock = threading.Lock()   # guards _cooldowns and _deauth_tracker
...
def _is_whitelisted(mac: str, cfg: dict) -> bool:
...
def _build_telegram_message(alert: dict) -> str:
    ...
    # parse_mode=HTML: every interpolated value MUST be escaped to prevent
    # attacker SSIDs/MACs from injecting markup or phishing links.
    type_label = html.escape(alert_type.replace("_", " ").title())
    node       = html.escape(str(alert.get("node", "unknown")))
    mac        = html.escape(str(alert.get("mac", "unknown")))
    vendor     = alert.get("vendor")
    ssid       = alert.get("ssid")
    rssi       = alert.get("rssi")
    ts         = _safe_ts(alert.get("timestamp"))

    mac_str  = f"{mac} [{html.escape(str(vendor))}]" if vendor else mac
    ...
    if ssid:
        lines.append(f"<b>SSID:</b>  <code>{html.escape(str(ssid))}</code>")
...
def _send_telegram(alert: dict, cfg: dict):
    ...
    if not _TG_TOKEN_RE.match(token):
        print("[Notifier] Telegram bot_token has an invalid format; refusing to send.")
        return
    url     = f"https://api.telegram.org/bot{token}/sendMessage"
    message = _build_telegram_message(alert)
    data    = {"chat_id": chat_id, "text": message, "parse_mode": "HTML"}
    try:
        resp = requests.post(url, json=data, timeout=5, allow_redirects=False)
        if resp.status_code == 200:
            print("[Notifier] Telegram alert sent.")
        else:
            print(f"[Notifier] Telegram error: {resp.status_code}")
...
def _send_discord(alert: dict, cfg: dict):
    ...
    if not _DISCORD_WEBHOOK_RE.match(webhook_url):
        print("[Notifier] Discord webhook_url is not a valid Discord webhook; refusing to send.")
        return
    data = _build_discord_embed(alert)
    try:
        resp = requests.post(webhook_url, json=data, timeout=5, allow_redirects=False)
        if resp.status_code in (200, 204):
            print("[Notifier] Discord alert sent.")
        else:
            print(f"[Notifier] Discord error: {resp.status_code}")
    except Exception as e:
...
def _notifier_worker():
    while True:
        ...
        try:
            ...
            # Never let one malformed alert terminate the only notifier thread.
...
def track_deauth(alert: dict, cfg: dict) -> bool:
    mac    = alert.get("mac")
    now    = time.time()
...
def test_discord(cfg: dict) -> bool:
    try:
        webhook_url = cfg.get("webhook_url", "")
        if not webhook_url or not _DISCORD_WEBHOOK_RE.match(webhook_url):
            return False
        data = {
            "embeds": [{
